...
- Rust 기반 세션 watch 구현
  ...
  - `sessions/`
- Rust 기반 코드 인덱서 구현
- Agent-Vis Bridge 프로토타입
  - runtime home discovery
  - SQLite thread reader
  - rollout JSONL normalizer
  - focus signal scoring
  - visual agent event ingest command
  - rule-based change cluster 생성
- React 기반 observer UI
  - 세션 목록
  - 이벤트 타임라인
  - 파일/디렉터리 중심 context graph
  - bridge event, focus, evidence detail panel
  - 브라우저 검증용 preview fallback
...
- watch 대상 계획, watcher 등록, Tauri 이벤트 emit, 프론트엔드 구독이 구현되어 있다.

남은 일:

- watch 이벤트 debounce 정책을 더 정교하게 조정
- 장시간 watch 오류 상태의 사용자 표시 강화
...
- session index reader: `state_5.sqlite`의 `threads` 테이블 기반 구현
- rollout reader: 선택 세션의 JSONL을 읽어 최신 이벤트 제한으로 정규화
- event normalizer: message/tool/command/patch/system/error 이벤트 구분
- active session scorer: workspace `cwd` 일치와 recency 기반 정렬
...
- React observer UI에서 세션별 timeline을 표시한다.
- watcher 이벤트를 받으면 snapshot을 갱신한다.
- diagnostics 패널에 fallback, stale runtime, load error를 표시한다.
...
- Rust workspace indexer가 directory/file/symbol/import graph를 만든다.
- React Flow 기반 context graph가 focus score, cluster, pin 상태를 시각화한다.
...
- Tauri `record_visual_agent_event` command가 placeholder visual event ingest endpoint 역할을 한다.
- bridge snapshot이 focus signal, visual agent event, change cluster, evidence event id를 함께 반환한다.
- UI detail panel이 visual event, focus, cluster evidence를 함께 표시한다.
...
- 디렉터리/파일 기본 그래프와 일부 언어의 symbol/import 추출이 구현되어 있다.
- MVP는 snapshot reload 방식이며, 증분 재인덱싱은 후속 개선으로 남아 있다.
